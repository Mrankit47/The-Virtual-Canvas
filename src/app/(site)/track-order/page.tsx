...
export default function TrackOrderPage() {
  ...
  // Step 6: Auto-Trigger Payment for high-conversion redirects
  useEffect(() => {
  ...
  // Step 1: Frontend Payment Button Trigger
  const handlePayment = async () => {
    ...
    if (!env.NEXT_PUBLIC_RAZORPAY_KEY || env.NEXT_PUBLIC_RAZORPAY_KEY.includes('YOUR_KEY_HERE')) {
      alert("Razorpay Public Key is missing or invalid in .env.local. Please check your configuration.");
      return;
    }
  ...
  const getStatusColor = (status: string) => {
    switch(status) {
      case 'progress': return 'bg-blue-500/10 text-blue-700 border-blue-500/20';
      case 'completed': return 'bg-green-500/10 text-green-700 border-green-500/20';
      case 'discussion': return 'bg-purple-500/10 text-purple-700 border-purple-500/20';
  ...
  // Step 5: Fix Process Status Mapping
  const stepMap: Record<string, number> = {
    pending: 1,
    discussion: 2,
    progress: 3,
    in_progress: 3, // Support variation from Sanity preview
    review: 4,
    completed: 5
  };
        ...
        <AnimatePresence mode="wait">
          {loading && (
            <motion.div key="loading" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="w-full space-y-4">
               ...
               <div className="w-full h-24 bg-ink/5 animate-pulse border border-ink/10"></div>
            </motion.div>
          )}
          
          {error && (
            <motion.div key="error" initial={{opacity:0, y:10}} animate={{opacity:1,y:0}} exit={{opacity:0}} className="w-full p-8 border border-red-500/30 bg-red-500/5 text-red-700 text-xs text-center font-medium shadow-sm flex flex-col items-center gap-4">
              <span>{error}</span>
               ...
               <div className="flex flex-col gap-6">
                 <div className="mb-4">
                   ...
                   <div className="w-full flex justify-between items-start relative px-2 md:px-0">
                     ...
                     {/* Step 8: Ensure status matches EXACTLY */}
                     {[
                       ...
                       return (
                         <div key={step.id} className="flex flex-col items-center gap-4 relative bg-transparent">
                           <motion.div 
                             ...
                             className={`w-5 h-5 md:w-6 md:h-6 rounded-full border-[3px] flex items-center justify-center ${active ? 'bg-ink border-transparent shadow-md' : 'bg-canvas border-ink/20'}`}
                           >
                             {active && <div className={`w-2 h-2 bg-white rounded-full ${pulse ? 'animate-ping' : ''}`}></div>}
                           </motion.div>
                           <span className={`text-[7px] md:text-[9px] uppercase tracking-widest font-bold text-center w-12 md:w-20 leading-relaxed transition-opacity duration-700 ${active ? 'opacity-100 text-ink' : 'opacity-40'}`}>
                             {step.label}
                           </span>
                         </div>
                       )
                 ...
                 <div className="flex justify-between items-center mt-4 bg-canvas/40 p-6 border border-ink/5">
                   <div>
                     <p className="text-[10px] uppercase tracking-widest opacity-50 mb-1">Agreed Value</p>
                   ...
                   <div className="text-right">
                     {order.paymentStatus === 'paid' ? (
                       <span className="text-[10px] uppercase tracking-widest font-bold text-green-600 bg-green-100 px-3 py-1 rounded-full border border-green-200">
                         ✓ Payment Confirmed
                       </span>
                     ) : (
                       <button 
                         onClick={handlePayment}
                         className="bg-ink text-canvas px-6 py-3 text-[10px] uppercase tracking-widest hover:bg-ink/80 transition-all shadow-lg active:scale-95"
                       >
                         Pay with Razorpay
                       </button>
                     )}
                 ...
                 <div className="grid grid-cols-2 gap-4">
                    <div className="p-4 border border-ink/10 bg-white/50">
                        <p className="text-[10px] uppercase tracking-widest opacity-50 mb-1">Payment Status</p>
                        <p className="text-xs font-bold uppercase tracking-tighter">
                            {order.paymentStatus === 'paid' ? 'Paid' : 'Unpaid (Action Required)'}
                        </p>
                    </div>
                    <div className="p-4 border border-ink/10 bg-white/50">
                        <p className="text-[10px] uppercase tracking-widest opacity-50 mb-1">Commissioned</p>
                 ...
                 {order.description && (
                   <div className="mt-4 pt-6 border-t border-ink/10">
                     <p className="text-[10px] uppercase tracking-widest opacity-50 mb-3">Project Brief</p>
                     <p className="text-sm leading-relaxed opacity-80 italic border-l-2 border-ink/20 pl-4">{order.description}</p>
                   </div>
                 )}
               </div>
            </motion.div>
